...
void glfemDrawVectors(femMesh *theMesh, double *vecX, double *vecY, double scale) {
    if (theMesh == NULL || vecX == NULL || vecY == NULL) return;

    int nNodes = theMesh->nodes->nNodes;
    double *X = theMesh->nodes->X;
    double *Y = theMesh->nodes->Y;
    ...
    for (int i = 0; i < nNodes; ++i) {
        glVertex2f(X[i], Y[i]);
        glVertex2f(X[i] + scale * vecX[i], Y[i] + scale * vecY[i]);
    }
    glEnd();

    // Optionnel : dessiner des petits points à l'extrémité
    glPointSize(2.0);
    glBegin(GL_POINTS);
    for (int i = 0; i < nNodes; ++i) {
        glVertex2f(X[i] + scale * vecX[i], Y[i] + scale * vecY[i]);
...
{
    double xmin = theNodes->X[0], xmax = theNodes->X[0];
    double ymin = theNodes->Y[0], ymax = theNodes->Y[0];
    for (int i = 1; i < theNodes->nNodes; ++i) {
        if (theNodes->X[i] < xmin) xmin = theNodes->X[i];
        if (theNodes->X[i] > xmax) xmax = theNodes->X[i];
        if (theNodes->Y[i] < ymin) ymin = theNodes->Y[i];
        if (theNodes->Y[i] > ymax) ymax = theNodes->Y[i];
    }

    double cx = (xmax + xmin) / 2.0 + panX;
    double cy = (ymax + ymin) / 2.0 + panY;
    double Lx = xmax - xmin;
    double Ly = ymax - ymin;

    double scale = fmax(Lx / w, Ly / h) / zoom;
    double margin = 10.0 * scale;

    glViewport(0, 0, w, h);
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    gluOrtho2D(cx - w * scale / 2 - margin,
               cx + w * scale / 2 + margin,
               cy - h * scale / 2 - margin,
               cy + h * scale / 2 + margin);
    glMatrixMode(GL_MODELVIEW);
