...
femProblem *femElasticityCreate(femGeo* theGeometry, double E, double nu, double rho, double g, femElasticCase iCase, femSolverType typeSolver, femRenumType renumType) {
    printf("Création du problème avec solveur : %d\n", typeSolver);
    femProblem *theProblem = malloc(sizeof(femProblem));
    theProblem->sigmaXX = NULL;
    theProblem->sigmaYY = NULL;
    theProblem->sigmaXY = NULL;
    theProblem->typeSolver = typeSolver;
    ...
    if (theProblem->typeSolver == FEM_NONE) {
        theProblem->systemNone = femNoneSystemCreate(size);
    } else if (typeSolver == FEM_BAND) {
        femMeshRenumber(theGeometry->theElements, renumType);
        int bandElem = femMeshComputeBand(theGeometry->theElements);
        int bandGlobal = 2 * bandElem;  // ⚠️ x2 car deux degrés de liberté par nœud !
        theProblem->systemBand = femBandSystemCreate(size, bandGlobal);
    } else {
        //printf("🧠 Solveur full\n");
    }
...
void femElasticityWrite(femProblem *theProblem, const char *filename) {
    ...
    if (theProblem->space->type == FEM_QUAD) { fprintf(file, "Type of element    :  Quadrilateral \n"); }
    else if (theProblem->space->type == FEM_TRIANGLE) { fprintf(file, "Type of element    :  Triangle \n"); }
    else if (theProblem->space->type == FEM_EDGE)     { fprintf(file, "Type of element    :  Edge \n"); }
    else                                             { fprintf(file, "Type of element    :  Undefined \n"); }
    ...
    for (int i = 0; i < theProblem->nBoundaryConditions; i++) {
        ...
        if (theCondition->type == DIRICHLET_X)       { fprintf(file, " Dirichlet-X        = %14.7e, %14.7e ", value, NAN); }
        else if (theCondition->type == DIRICHLET_Y)  { fprintf(file, " Dirichlet-Y        = %14.7e, %14.7e ", value, NAN); }
        else if (theCondition->type == NEUMANN_X)    { fprintf(file, " Neumann-X          = %14.7e, %14.7e ", value, NAN); }
        else if (theCondition->type == NEUMANN_Y)    { fprintf(file, " Neumann-Y          = %14.7e, %14.7e ", value, NAN); }
...
femNoneSystem *femNoneSystemCreate(int size) {
    femNoneSystem *theSystem = malloc(sizeof(femNoneSystem));
    theSystem->size = size;
    theSystem->B = malloc(sizeof(double) * size);
    theSystem->X = malloc(sizeof(double) * size);
    double *data = malloc(sizeof(double) * size * size);
    theSystem->A = malloc(sizeof(double*) * size);
    for (int i = 0; i < size; ++i)
        theSystem->A[i] = data + i * size;
...
void femNoneSystemFree(femNoneSystem *theSystem) {
    free(theSystem->A[0]);  // bloc de données
    free(theSystem->A);     // tableau de pointeurs
    free(theSystem->B);
    free(theSystem->X);
    free(theSystem);
}

void femNoneSystemConstrain(femNoneSystem *theSystem, int myNode, double myValue) {
    for (int i = 0; i < theSystem->size; i++) {
        theSystem->B[i] -= myValue * theSystem->A[i][myNode];
        theSystem->A[i][myNode] = 0.0;
    }

    for (int j = 0; j < theSystem->size; j++)
        theSystem->A[myNode][j] = 0.0;

    theSystem->A[myNode][myNode] = 1.0;
    theSystem->B[myNode] = myValue;
}

double *femNoneSystemEliminate(femNoneSystem *theSystem) {
    // Pour l’instant, copie simple de la version full
    int size = theSystem->size;
    double **A = theSystem->A;
    double *B = theSystem->B;

    for (int k = 0; k < size; k++) {
        if (fabs(A[k][k]) < 1e-16) Error("Pivot trop petit dans None solver !");
...
void femSystemAdd(femProblem *theProblem, int i, int j, double value) {
    if (theProblem->typeSolver == FEM_FULL) {
        ...
        if (colBandIndex >= 0 && colBandIndex < 2 * band + 1)
            s->A[i][colBandIndex] += value;
        else
            printf("⚠️  [femSystemAdd] (i=%d, j=%d) out of band range (band=%d)\n", i, j, band);
    } else {
        Error("Solveur non reconnu dans femSystemAdd !");
    }
}


void femSystemAddRightHandSide(femProblem *theProblem, int i, double value) {
    if (theProblem->typeSolver == FEM_FULL) {
        femFullSystem *s = theProblem->system;
        s->B[i] += value;
    } else if (theProblem->typeSolver == FEM_NONE) {
        ...
        Error("Solveur non reconnu dans femSystemAddRightHandSide !");
    }
}

void femSystemConstraint(femProblem *theProblem, int index, double value) {
    switch (theProblem->typeSolver) {
        case FEM_FULL:
            femFullSystemConstrain(theProblem->system, index, value);
            break;
        case FEM_NONE:
            femNoneSystemConstrain((femNoneSystem *)theProblem->systemNone, index, value);
            break;
        case FEM_BAND:
            femBandSystemConstrain(theProblem->systemBand, index, value);
            break;
        default:
            Error("Type de solveur inconnu ou non supporté pour femSystemConstrain.");
    }
}
...
void femMeshRenumber(femMesh *theMesh, femRenumType renumType) {
    ...
    if (theNodes->number == NULL) {
        theNodes->number = malloc(sizeof(int) * nNodes);
        if (theNodes->number == NULL)
            Error("Échec allocation number dans femMeshRenumber !");
...
void femBandSystemAssemble(femBandSystem* myBandSystem, double *Aloc, double *Bloc, int *map, int nLoc) {
    int i, j, myRow, myCol, band = myBandSystem->band;
    for (i = 0; i < nLoc; i++) { 
        myRow = map[i];
        myBandSystem->B[myRow] += Bloc[i];
        for (j = 0; j < nLoc; j++) {
            myCol = map[j];
            int colBandIndex = myCol - myRow + band;
            if (colBandIndex >= 0 && colBandIndex < 2 * band + 1) {
                myBandSystem->A[myRow][colBandIndex] += Aloc[i * nLoc + j];
            } else {
                printf("[Assemble] colBandIndex out of bounds: row = %d, col = %d, index = %d (band = %d)\n",
                       myRow, myCol, colBandIndex, band);
            }
        }
    }
}
...
double *femBandSystemEliminate(femBandSystem *myBand) {
    double **A = myBand->A, *B = myBand->B;
    int size = myBand->size, band = myBand->band;
    int i, j, k, jmax;
    double factor;

    printf("[Eliminate] Début avec size = %d, band = %d\n", size, band);

    for (k = 0; k < size; k++) {
        if (fabs(A[k][band]) <= 1e-8) {
            printf("Pivot nul détecté à k = %d (pivot = %e)\n", k, A[k][band]);
            Error("Cannot eliminate with such a pivot");
        }
        jmax = fmin(k + band, size - 1);
        for (i = k + 1; i <= jmax; i++) {
            factor = A[k][i - k + band] / A[k][band];
            for (j = i; j <= jmax; j++) {
    ...
    for (i = size - 1; i >= 0; i--) {
        factor = 0.0;
        jmax = fmin(i + band, size - 1);
        for (j = i + 1; j <= jmax; j++) {
            factor += A[i][j - i + band] * B[j];
        }
        B[i] = (B[i] - factor) / A[i][band];
    }

    memcpy(myBand->X, B, size * sizeof(double));
    printf("[Eliminate] Terminé sans erreur\n");
    return myBand->X;
}

femBandSystem* femBandSystemCreate(int size, int band) {
    printf("[Create] Solveur bande utilisé - size = %d, band = %d\n", size, band);
    femBandSystem *theSystem = malloc(sizeof(femBandSystem));
    if (!theSystem) Error("Allocation échouée pour femBandSystem");
    theSystem->size = size;
    theSystem->band = band;
    theSystem->B = malloc(size * sizeof(double));
    theSystem->X = malloc(size * sizeof(double));
    theSystem->A = malloc(size * sizeof(double *));
    if (!theSystem->B || !theSystem->X || !theSystem->A) Error("Allocation échouée pour B, X ou A");

    for (int i = 0; i < size; i++) {
        theSystem->A[i] = malloc((2 * band + 1) * sizeof(double));
        if (!theSystem->A[i]) {
            printf("Échec alloc A[%d] avec taille %d\n", i, 2 * band + 1);
            Error("Allocation échouée pour A[i]");
        }
    }
...
void femBandSystemInit(femBandSystem *myBandSystem) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        for (int j = 0; j < 2*band+1; j++) {
            myBandSystem->A[i][j] = 0;
        }
    }
}

void femBandSystemConstrain(femBandSystem *myBand, int myNode, double value) {
    int i;
    int band = myBand->band;
    int size = myBand->size;

    if (myNode < 0 || myNode >= size) {
        printf("[Constrain] myNode = %d is out of bounds!\n", myNode);
        Error("Index invalide dans femBandSystemConstrain");
    }

    for (i = fmax(0, myNode - band); i < fmin(size, myNode + band + 1); ++i) {
        myBand->B[i] -= myBand->A[i][myNode - i + band] * value;
    }

    for (i = 0; i < 2 * band + 1; ++i) {
        myBand->A[myNode][i] = 0.0;
    }

    myBand->A[myNode][band] = 1.0;
    myBand->B[myNode] = value;
}

double *femElasticityUnmapSoluce(femProblem *theProblem) {
    int n = theProblem->geometry->theNodes->nNodes;
    int *number = theProblem->geometry->theNodes->number;
    double *soluce = theProblem->soluce;

    double *unmapped = malloc(sizeof(double) * 2 * n);
    if (unmapped == NULL)
        Error("Allocation failed for unmapped solution");

    // Création du tableau inverseNumber
    int *inverseNumber = malloc(sizeof(int) * n);
    if (inverseNumber == NULL)
        Error("Allocation failed for inverseNumber");

    for (int i = 0; i < n; ++i)
        inverseNumber[number[i]] = i;

    for (int i = 0; i < n; ++i) {
        int originalIndex = inverseNumber[i];
        unmapped[2*originalIndex]   = soluce[2*i];
        unmapped[2*originalIndex+1] = soluce[2*i+1];
    }
...
double *femElasticityUnmapResiduals(femProblem *theProblem) {
    int n = theProblem->geometry->theNodes->nNodes;
    int *number = theProblem->geometry->theNodes->number;
    double *res = theProblem->residuals;

    double *unmapped = malloc(sizeof(double) * 2 * n);
    if (unmapped == NULL)
        Error("Allocation failed for unmapped residuals");

    for (int i = 0; i < n; ++i) {
        int newIndex = number[i];
        unmapped[2*i]   = res[2*newIndex];
        unmapped[2*i+1] = res[2*newIndex+1];
    }
    return unmapped;
}
